...
class StreamDj():
    ...
    def __init__(self, channel_name:str, author_name:str=None):
        ...
        self._send_url_template = 'https://streamdj.ru/includes/back.php?func=add_track&channel=%s'
    ...
    def send(self, video_url:str) -> dict:
        ...
        responce = requests.post(self._send_url_template % self._channel_id, data=data, timeout=10)

        if responce.status_code != 200:
            raise ConnectionError(
                f'try to send {video_url} to {self._channel_name}, status_code={responce.status_code}.'
            )
